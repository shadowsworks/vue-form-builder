<template>
  <div class="item-viewer text-left" v-if="state_data.loaded">
    <template v-if='bind_data.item_data.item_type=="text" || bind_data.item_data.item_type=="texts" || bind_data.item_data.item_type=="number" || 
      ...
      bind_data.item_data.item_type=="markdown" || bind_data.item_data.item_type=="image" ' >
    ...
    <template v-if='bind_data.item_data.item_type=="number"'>
      <div class="mx-2">{{ bind_data.item_data.item_data }} {{ bind_data.item_data.item_unit_name }}</div>
    </template>
    ...
    <template v-if='bind_data.item_data.item_type=="checkbox"'>
      <div class="mx-2 mb-1" v-for="(item_data) in bind_data.item_data.item_data" :key="item_data">
        {{ item_data }}
      </div>
    </template>
    ...
    <template v-if='bind_data.item_data.item_type=="name"' >
      <b-row>
        <b-col>
          ...
          <div class="mx-2">{{ bind_data.item_data.item_data[0] }}</div>
        </b-col>
        <b-col>
          <label class="text-secondary mt-2 mb-0 small" >{{ bind_data.item_data.item_name[1] }}</label>
          <div class="mx-2">{{ bind_data.item_data.item_data[1] }}</div>
        </b-col>
      </b-row>
    </template>
    ...
    <template v-if='bind_data.item_data.item_type=="telephone"' >
      <div class="mx-2">{{ bind_data.item_data.item_data[0] }} - {{ bind_data.item_data.item_data[1] }} - {{ bind_data.item_data.item_data[2] }}</div>
    </template>

    <!-- 日付 -->
    <template v-if='bind_data.item_data.item_type=="date"'>
      <div class="mx-2">{{ set_date_format(bind_data.item_data.item_data) }}</div>
    </template>

    <!-- 時刻 -->
    <template v-if='bind_data.item_data.item_type=="time"'>
      <div class="mx-2">{{ set_time_format(bind_data.item_data.item_data) }}</div>
    </template>
    
    <!-- 日時 -->
    <template v-if='bind_data.item_data.item_type=="datetime"' >
      <div class="mx-2">{{ set_date_format(bind_data.item_data.item_data[0]) }} {{ set_time_format(bind_data.item_data.item_data[1]) }}</div>
    </template>
